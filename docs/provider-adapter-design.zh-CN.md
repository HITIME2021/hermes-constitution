...
## Hermes self-edit 默认禁用

Hermes 是 orchestrator，不是默认 coder。本地 Hermes agent 个性化修改有时是必要的，
但通过 Hermes 自身 model backend 进行 self-edit 默认禁用。
...
Hermes 不得通过自身 model/tool loop 直接 patch、rewrite、create 或 delete 代码文件来
完成 implementation work。即使目标是本地 Hermes agent 个性化修改，也适用该规则。

只有操作者对具体任务明确给出 emergency override 时，才允许 Hermes self-edit。该
override 必须是 task-specific，并且同时包含：
...
- 操作者明确说本任务允许 Hermes self-edit
- evidence 记录这是 emergency local self-edit override

没有该明确 override 时，Hermes 必须把 implementation work 路由到 provider
orchestration。

即使存在宽泛的 local customization 请求，以下情况仍禁止 Hermes self-edit：
