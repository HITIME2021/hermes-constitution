...
User Intent
  ...
  -> Final Report
  -> MemoryCandidate 或已批准的 Memory Writeback
```
...
Frontend Tool output 是 input evidence，不是 authority。Backend Tool execution
是 authority-bearing effect，必须有 scope control。Hermes 通过 `ExecutionRequest`、
review gates、stop conditions 和 operator approval 保持最终执行权威。

Hermes 不应该简单地把任务交给某个 Agent，而应该先判断任务类型、工具模式、artifact
状态、风险、scope、Provider 能力和审查等级。
